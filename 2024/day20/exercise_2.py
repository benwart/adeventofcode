...
def get_cheats(map: list[list[str]]) -> Iterable[list[tuple[int, int, str]]]:
    # compute cheats in concentric diamonds up to 20 steps away in all directions
    # generate a simple h+v path for each location as a cheat is defined by start and end not path
    # yield the path including start and end along with the original data for each step
