...
def do_lookup(t, w, x, x_offset, y, y_offset):
    if x < 0 or y < 0 or x == width or y == (len(t) // w):
        return 0
    status = t[(y * w) + x]
    if status == SeatStatus.TAKEN:
        return 1
    elif status == SeatStatus.EMPTY:
        return 0
    else:
        return do_lookup(t, w, x + x_offset, x_offset, y + y_offset, y_offset)
...
def check_seat(t, w, i):
    ...
    adjacent = {
        "top-left": do_lookup(t, w, x - 1, -1, y - 1, -1)
        if not top and not left
        else 0,
        "top-middle": do_lookup(t, w, x, 0, y - 1, -1) if not top else 0,
        "top-right": do_lookup(t, w, x + 1, 1, y - 1, -1)
        if not top and not right
        else 0,
        "middle-left": do_lookup(t, w, x - 1, -1, y, 0) if not left else 0,
        "middle-right": do_lookup(t, w, x + 1, 1, y, 0) if not right else 0,
        "bottom-left": do_lookup(t, w, x - 1, -1, y + 1, 1)
        if not bottom and not left
        else 0,
        "bottom-middle": do_lookup(t, w, x, 0, y + 1, 1) if not bottom else 0,
        "bottom-right": do_lookup(t, w, x + 1, 1, y + 1, 1)
