...
class Monkey:
    ...
    def inspect_item(self, item: int) -> int:
        self.inspections += 1
        return self.op.operate(old=item)
    ...
    for round in range(rounds):
        ...
        for monkey in monkeys:
            ...
            while monkey.items:
                ...
                # relief = old / 3 rounded down to nearest integer
                item = relief_adjustment(item)

                # test and throw item = check divisible by value then throw if true/false
                monkey.test_item(item, monkeys)

        for monkey in monkeys:
            print(f"Monkey {monkey.id}: {', '.join([str(i) for i in monkey.items])}")

        print("")

    for monkey in monkeys:
        print(f"Monkey {monkey.id} inspected items {str(monkey.inspections)} times.")
