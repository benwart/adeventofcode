...
INSTSTRUCTION_REGEX = compile(r"([UDLR]) (\d+) \(#(?P<steps>[a-f0-9]{5})(?P<direction>[0-3])\)")
...
class Instruction:
    ...
    def __post_init__(self, line: str):
        ...
        self.direction = direction_map[match.group("direction")]
        self.steps = int(f"0x{match.group("steps")}", 16)
