...
class Path(object):
    def __init__(self, nodes: list[Node]):
        self.nodes = nodes
        self.double_limited = False
        self.limited_visits = set()

        for n in nodes:
            if n.is_limited():
                if n in self.limited_visits and not self.double_limited:
                    self.double_limited = True
                else:
                    self.limited_visits.add(n)
...
def follow_paths(p: Path):
    ...
    for e in p.tail.edges:
        ...
        if n not in p.limited_visits:
            # find longer paths
            paths |= follow_paths(p.add_node(n))

        elif not p.double_limited:
            # second attempt at a single limited  node
            paths |= follow_paths(p.add_node(n))
